      ...
      >
        {preview ? (
          ...
          <div className="flex flex-col items-center gap-1 text-gray-400">
            ...
            <span className="text-[10px]">PNG or JPG</span>
          </div>
        )}
...
// ─────────────────────────────────────────────────────────────
// Main Step7Signatures component
// ─────────────────────────────────────────────────────────────
export default function Step7Signatures() {
  ...
  // Build the same cleanPayload merge logic page.tsx uses, but simplified —
  // we just need enough to generate a readable preview. We pass all wizard
  // values that exist so far.
  useEffect(() => {
    let cancelled = false;
    ...
    async function generatePreview() {
      ...
      try {
        // Flatten all wizard step values into one object
        const flat: Record<string, any> = {};
        Object.values(values || {}).forEach((stepVals: any) => {
          if (stepVals && typeof stepVals === "object") {
            Object.assign(flat, stepVals);
          }
        });
  ...
  // Only run once on mount — the PDF is a preview of what's been filled so far
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Two-column layout: PDF left, signature panel right */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-3">
          ...
          <div className="relative rounded-xl bg-gray-50 border border-gray-100 overflow-hidden">
            {pdfLoading && (
              <div className="flex flex-col items-center justify-center h-[500px] gap-3 text-gray-400">
            ...
            {pdfError && !pdfLoading && (
              <div className="flex flex-col items-center justify-center h-[500px] gap-2 text-gray-400">
