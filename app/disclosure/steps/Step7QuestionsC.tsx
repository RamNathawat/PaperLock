...
function YesNoRow({ num }: { num: number }) {
  ...
  return (
    <div className={`relative rounded-2xl border p-5 space-y-4 ${hasError ? "border-amber-200 bg-amber-50/40" : "border-gray-100 bg-gray-50/30"}`}>
      ...
      {value === "YES" && (
        <textarea {...register(`questionComments.${num}`, { required: true, shouldUnregister: true })} rows={3} placeholder={`Add details for Q${num}…`}
          className={`w-full rounded-xl border px-4 py-3 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all ${submitCount > 0 && !!(errors as any)?.questionComments?.[num] ? "border-amber-300 bg-amber-50" : "border-gray-200"}`}
        />
      )}
    </div>
  );
